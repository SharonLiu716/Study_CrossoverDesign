...
#true mean
tm=[np.exp(alpha+eta1), np.exp(alpha + gamma1), np.exp(alpha+eta2+gamma2),
    np.exp(alpha+delta1), np.exp(alpha+eta2+gamma1+delta1),np.exp(alpha+eta1+gamma2+delta1),
    np.exp(alpha+eta1+delta2), np.exp(alpha+ eta2+gamma1+delta2), np.exp(alpha+gamma2+delta2)]
del alpha, eta1, eta2, gamma1, gamma2, delta1, delta2 
pi=50/150
I = pi*np.array([[sum(tm), tm[0]+tm[5]+tm[6], tm[2]+tm[4]+tm[7], tm[1]+tm[4]+tm[7], tm[2]+tm[5]+tm[8], sum(tm[3:6]), sum(tm[6:9])],
                ...
                [tm[2]+tm[5]+tm[8], tm[5], tm[2] , 0, tm[2]+tm[5]+tm[8], tm[7], tm[8] ],
                [sum(tm[3:6]), tm[5],tm[4], tm[4], tm[7], sum(tm[3:6]), 0],
...
def Estimate(mean_true,sample_size):
    ...
    def L(params):
        ...
        f1,f2,f3=0,0,0
        for i in range(len(data)):
            f1=f1+(alpha+eta1)*data.at[i,'Yi11']+(alpha+gamma1)*data.at[i,'Yi21']+(alpha+eta2+gamma2)*data.at[i,'Yi31']-np.exp(alpha+eta1)-np.exp(alpha+gamma1)-np.exp(alpha+eta2+gamma2)
            f2=f2+(alpha+delta1)*data.at[i,'Yi12']+(alpha+eta2+gamma1+delta1)*data.at[i,'Yi22']+(alpha+eta1+gamma2+delta1)*data.at[i,'Yi32']-np.exp(alpha+delta1)-np.exp(alpha+eta2+gamma1+delta1)-np.exp(alpha+eta1+gamma2+delta1)
            f3=f3+(alpha+eta1+delta2)*data.at[i,'Yi13']+(alpha+eta2+gamma1+delta2)*data.at[i,'Yi23']+(alpha+gamma2+delta2)*data.at[i,'Yi33']-np.exp(alpha+eta1+delta2)-np.exp(alpha+eta2+gamma1+delta2)-np.exp(alpha+gamma2+delta2)
        return f1+f2+f3
    initial_guess = np.array([0.95, 0.302, 0.354, 1.198, 1.258, 0.206, 0.264])
    solve = scipy.optimize.minimize (lambda params: -L(params), initial_guess)
    MLE_opt=solve.x
    ...
    '''Mean'''
    etimate_of_mean = pd.DataFrame({'Mean_11': np.exp(MLE_opt[0]+MLE_opt[1]), 
                                    'Mean_21': np.exp(MLE_opt[0] + MLE_opt[3]), 
                                    'Mean_31': np.exp(MLE_opt[0] + MLE_opt[2] + MLE_opt[4]),
                                    'Mean_12': np.exp(MLE_opt[0] + MLE_opt[5]), 
                                    'Mean_22': np.exp(MLE_opt[0] + MLE_opt[2] + MLE_opt[3] + MLE_opt[5]),
                                    'Mean_32': np.exp(MLE_opt[0] + MLE_opt[1] + MLE_opt[4] + MLE_opt[5]),
                                    'Mean_13': np.exp(MLE_opt[0] + MLE_opt[1] + MLE_opt[6]), 
                                    'Mean_23': np.exp(MLE_opt[0] + MLE_opt[2] + MLE_opt[3] + MLE_opt[6]),
                                    'Mean_33': np.exp(MLE_opt[0] + MLE_opt[4] + MLE_opt[6])},index=[0])
    
    sm=list(etimate_of_mean.loc[0,:])    
    '''Estimate matrix I'''
    p1=50/150
    I_hat =p1*np.array([  [sum(sm), sm[0]+sm[5]+sm[6], sm[2]+sm[4]+sm[7], sm[1]+sm[4]+sm[7], sm[2]+sm[5]+sm[8], sum(sm[3:6]), sum(sm[6:9])],
                    [sm[0]+sm[5]+sm[6], sm[0]+sm[5]+sm[6], 0, 0, sm[5], sm[5], sm[6] ],
                    [sm[2]+sm[4]+sm[7], 0, sm[2]+sm[4]+sm[7], sm[4]+sm[7], sm[2], sm[4], sm[7] ],
                    [sm[1]+sm[4]+sm[7], 0, sm[4]+sm[7], sm[1]+sm[4]+sm[7], 0, sm[4], sm[7]],
                    [sm[2]+sm[5]+sm[8], sm[5], sm[2] , 0, sm[2]+sm[5]+sm[8], sm[7], sm[8] ],
                    [sum(sm[3:6]), sm[5],sm[4], sm[4], sm[7], sum(sm[3:6]), 0],
                    [sum(sm[6:9]), sm[6], sm[7], sm[7], sm[8], 0, sum(sm[6:9])]
                    ])
    ...
    V_hat=p1*np.array([
        [sum(Var), Var[0]+Cov[0]+Cov[1]+Var[5]+Cov[4]+Cov[5]+Var[6]+Cov[6]+Cov[8],Var[2]+Cov[1]+Cov[2]+Var[4]+Cov[3]+Cov[5]+Var[7]+Cov[6]+Cov[8], Var[1]+Cov[0]+Cov[2]+Var[4]+Cov[3]+Cov[5]+Var[7]+Cov[6]+Cov[8],Var[2]+Cov[1]+Cov[2]+Var[5]+Cov[4]+Cov[5]+Var[8]+Cov[7]+Cov[8],sum(Var[3:6])+2*sum(Cov[3:6]),sum(Var[6:9])+2*sum(Cov[6:9])],
        [Var[0]+Cov[0]+Cov[1]+Var[5]+Cov[4]+Cov[5]+Var[6]+Cov[6]+Cov[8],Var[0]+Var[5]+Var[6],Cov[1]+Cov[5]+Cov[6],Cov[0]+Cov[5]+Cov[6],Var[5]+Cov[1]+Cov[7],Var[5]+Cov[4]+Cov[5],Var[6]+Cov[6]+Cov[7]],
        [Var[2]+Cov[1]+Cov[2]+Var[4]+Cov[3]+Cov[5]+Var[7]+Cov[6]+Cov[8],Cov[1]+Cov[5]+Cov[6],Var[2]+Var[4]+Var[7],Var[4]+Cov[2]+Cov[7],Var[2]+Cov[5]+Cov[8],Var[4]+Cov[5]+Cov[3],Var[7]+Cov[6]+Cov[8]],
        [Var[1]+Cov[0]+Cov[2]+Var[4]+Cov[3]+Cov[5]+Var[7]+Cov[6]+Cov[8],Cov[0]+Cov[5]+Cov[6],Var[4]+Cov[2]+Cov[7],Var[1]+Var[4]+Var[7],Cov[2]+Cov[5]+Cov[8],Var[4]+Cov[3]+Cov[5],Var[7]+Cov[6]+Cov[8] ],
        [Var[2]+Cov[1]+Cov[2]+Var[5]+Cov[4]+Cov[5]+Var[8]+Cov[7]+Cov[8],Var[5]+Cov[1]+Cov[7],Var[2]+Cov[5]+Cov[8],Cov[2]+Cov[5]+Cov[8],Var[2]+Var[5]+Var[8],Var[5]+Cov[4]+Cov[5],Var[8]+Cov[7]+Cov[8]],
        [sum(Var[3:6])+2*sum(Cov[3:6]),Var[5]+Cov[4]+Cov[5],Var[4]+Cov[5]+Cov[3],Var[4]+Cov[3]+Cov[5],Var[5]+Cov[4]+Cov[5],sum(Var[3:6])+2*sum(Cov[3:6]),0],
...
[sum(Var), sum(Var[0:2])+2*Cov[0]+Cov[1]+Cov[2]+Var[5]+Cov[4]+Cov[5], Var[4]+Cov[3]+Cov[5]+Var[6]+Var[8]+sum(Cov[6:9])+Cov[7], Var[1]+Cov[0]+Cov[2]+Var[4]+Cov[3]+Cov[5]+Var[7]+Cov[6]+Cov[8], Var[2]+Cov[1]+Cov[2]+Var[5]+Cov[4]+Cov[5]+Var[8]+Cov[7]+Cov[8], sum(Var[3:6])+2*sum(Cov[3:6]), sum(Var[6:9])+2*sum(Cov[6:9])],
[sum(Var[0:2])+2*Cov[0]+Cov[1]+Cov[2]+Var[5]+Cov[4]+Cov[5],sum(Var[0:2])+2*Cov[0]+Var[5], Cov[5], Var[1]+Cov[0]+Cov[5], Cov[1]+Cov[2]+Cov[5], Var[5]+Cov[4]+Cov[5], 0],
[Var[4]+Cov[3]+Cov[5]+Var[6]+Var[8]+sum(Cov[6:9])+Cov[7],Cov[5],Var[4]+Var[6]+Var[8]+2*Cov[7], Var[4]+Cov[6]+Cov[8], Var[8]+Cov[5]+Cov[7], Var[4]+Cov[3]+Cov[5], Var[6]+Var[8]+sum(Cov[6:9])],
[Var[1]+Cov[0]+Cov[2]+Var[4]+Cov[3]+Cov[5]+Var[7]+Cov[6]+Cov[8],Var[1]+Cov[0]+Cov[5],Var[4]+Cov[6]+Cov[8], Var[1]+Var[4]+Var[5], Cov[2]+Cov[5]+Cov[8] , Var[4]+Cov[3]+Cov[5] ,Var[7]+Cov[6]+Cov[8] ],
[Var[2]+Cov[1]+Cov[2]+Var[5]+Cov[4]+Cov[5]+Var[8]+Cov[7]+Cov[8],Cov[1]+Cov[2]+Cov[5],Var[8]+Cov[5]+Cov[7], Cov[2]+Cov[5]+Cov[8], Var[2]+Var[5]+Var[8] , Var[5]+Cov[4]+Cov[5], Var[8]+Cov[7]+Cov[8]],
[sum(Var[3:6])+2*sum(Cov[3:6]),Var[5]+Cov[4]+Cov[5],Var[4]+Cov[3]+Cov[5],Var[4]+Cov[3]+Cov[5] , Var[5]+Cov[4]+Cov[5], sum(Var[3:6])+2*sum(Cov[3:6]), 0],
[sum(Var[6:9])+2*sum(Cov[6:9]),0,Var[6]+Var[8]+sum(Cov[6:9]),Var[7]+Cov[6]+Cov[8],Var[8]+Cov[7]+Cov[8], 0, sum(Var[6:9])+2*sum(Cov[6:9])]
