...
for (i in 1:sim_time){
  ...
  y13<-rpois(seq, lambda = mean.true[3])
  y21<-rpois(seq, lambda = mean.true[4])
  y22<-rpois(seq, lambda = mean.true[5])
  y23<-rpois(seq, lambda = mean.true[6])
  Y <- c(y11,y12,y13,y21,y22,y23)
  X = c(rep(0,seq), rep(1,3*seq), rep(0,2*seq))
