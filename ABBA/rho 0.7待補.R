...
# - X、Z、G:自變量，用來fit glm
#===========================================================
sim_time=5000;cor_par=0.8;seq=150
...
#   print(seq)
  ...
  for (i in 1:sim_time){
    ...
    LR.rbup[i] = uniroot(LR.rb, c(eta[i],eta[i]+0.3))$root
    ...
    S.naup[i] = uniroot(S.na, c(eta[i],eta[i]+0.3))$root
    S.nalw[i] = uniroot(S.na, c(eta[i]-0.3,eta[i]))$root
...
library(readr)
write_csv(data.frame(df), "C:\\Users\\User\\Documents\\Study_CrossoverDesign\\ABBA\\rho8N=300.csv")
