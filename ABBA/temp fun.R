...
ABBA.sim<-function(seq){
  ...
  for (i in 1:sim_time){
    ...
    W.naup[i]<-uniroot(W.na,c(eta[i],eta[i]+0.5))$root
    W.nalw[i]<-uniroot(W.na,c(eta[i]-0.5,eta[i]))$root
    ...
    W.rbup[i]<-uniroot(W.rb,c(eta[i],eta[i]+0.5))$root
    W.rblw[i]<-uniroot(W.rb,c(eta[i]-0.5,eta[i]))$root
  ...
  #var
  V.std<-c(sv = 2*seq*var(eta), na = mean(var.na), rb = mean(var.rb),s = sqrt(var(eta)), na.std = sqrt(mean(var.na)), rb.std = sqrt(mean(var.rb)))
  ...
  df = c( N = seq, MLE = mle, VarStd = V.std, a = Alpha, Wna = Wna.CI, Wrb = Wrb.CI ,LRna =LRna.CI, LRrb=LRrb.CI, Sna =Sna.CI, LRrb=Srb.CI )
...
res<-lapply(c(25,50,75,100,125,150), ABBA.sim)

write_csv(data.frame(res), "C:\\Users\\User\\Documents\\Study_CrossoverDesign\\ABBA\\rho_0.4.csv")

# mm<-function(x){
#   alpha<-c(Srb = mean(c(1,3,5)),Sna = mean(c(2,4,6)),LRrb = mean(c(1,1,1)),Srb = mean(c(3,3,3)))
#   mle<-c(tao = 1.0 ,eta = 0, gam = 1.2, del = 0.2)
#   df=c(A=alpha,M=mle)
#   return(df)
# }
# res<-lapply(c(25,50,75,100,125,150), mm)
