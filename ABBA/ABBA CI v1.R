...
# - X、Z、G:自變量，用來fit glm
#===========================================================
sim_time=5000;seq=25;cor_par=0.4
...
for (i in 1:sim_time){
  ...
  y1 <- genCorGen(seq, nvars = 2, params1 = l1, dist = "poisson", rho = cor_par, corstr = "cs", wide = TRUE,cnames='y11,y12')
  y1 <-as.matrix(y1[,c('y11','y12')])
  y2 <- genCorGen(seq, nvars = 2, params1 = l2, dist = "poisson", rho = cor_par, corstr = "cs", wide = TRUE,cnames='y21,y22')
  ...
  S.rbup[i] = uniroot(S.rb, c(eta[i],eta[i]+0.3))$root
  S.rblw[i] = uniroot(S.rb, c(eta[i]-0.3,eta[i]))$root
  Srb.len[i] = S.rbup[i] - S.rblw[i]
  Srb.cp[i] = ifelse((S.rblw[i] < 0 & S.rbup[i] > 0), 1, 0)
