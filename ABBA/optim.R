...
# - X、Z、G:自變量，用來fit glm
#===========================================================
sim_time=5000;cor_par=0.7;seq=100
...
for (i in 1:sim_time){
  ...
  LR.naup[i] = optim(0.001, LR.na,method = 'Brent',lower = l, upper = u)$par
  LR.nalw[i] = optim(0.001, LR.na,method = 'Brent',lower = -u, upper = l)$par
  ...
  # optim(0.001, LR.na,method = 'Brent',lower = 0.01, upper = 0.6)$par
  # optim(0.001, LR.na,method = 'Brent',lower = -0.6, upper = 0.01)$par
   LR.rbup[i] = optim(0.001, LR.rb,method = 'Brent',lower = l, upper = u)$par
   LR.rblw[i] = optim(0.001, LR.rb,method = 'Brent',lower = -u, upper = l)$par
  ...
  S.naup[i] = optim(0.001, S.na,method = 'Brent',lower = l, upper = u)$par
  S.nalw[i] = optim(0.001, S.na,method = 'Brent',lower = -u, upper = l)$par
  ...
  S.rbup[i] = optim(0.001, S.rb,method = 'Brent',lower = l, upper = u)$par
  S.rblw[i] = optim(0.001, S.rb,method = 'Brent',lower = -u, upper = l)$par
  ...
  W.naup[i]<-optim(0.001, W.na,method = 'Brent',lower = l, upper = u)$par
  W.nalw[i]<-optim(0.001, W.na,method = 'Brent',lower = -u, upper = l)$par
  ...
  W.rbup[i]<-optim(0.001, W.rb,method = 'Brent',lower = l, upper = u)$par
  W.rblw[i]<-optim(0.001, W.rb,method = 'Brent',lower = -u, upper = l)$par
...
library(readr)
write_csv(data.frame(df), "C:\\Users\\User\\Documents\\Study_CrossoverDesign\\ABBA\\rho0.7N=200.csv")
