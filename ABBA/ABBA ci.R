...
for (i in 1:sim_time){
  ...
  W.mu = Vectorize( function(e) return( (e-eta[i])^2 ))
  
  curve(W.mu,-5,5, n = 1000, lwd =3, col = "blue", cex.axis = 2, cex.lab = 1.5, main =  expression(paste("Wald function of ", e)),
        xlab = ~e, ylab = "Wald")
  ...
  c(uniroot(W.muC,c(eta[i]-5,eta[i]))$root,uniroot(W.muC,c(eta[i],eta[i]+5))$root)
  #=================================================
  #log LR test :par=exp(.)
  #=================================================
  lik<-function(par){ 
